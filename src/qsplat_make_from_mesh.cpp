	      ...
	      std::string &comments)
{
	...
	int other_prop_len, color_offset = 0;
	char buf[255];
	int i, result;
...
#define LINE_IS(text) !strncasecmp(buf, text, strlen(text))
	...
	GET_LINE();
	if (LINE_IS("format binary_little_endian 1.0")) {
		little_endian = true;
	} else if (LINE_IS("format binary_big_endian 1.0")) {
		little_endian = false;
	} else {
...
#ifndef WE_ARE_LITTLE_ENDIAN
			DO_SWAP_FLOAT(leaves[i].pos[0]);
			DO_SWAP_FLOAT(leaves[i].pos[1]);
			DO_SWAP_FLOAT(leaves[i].pos[2]);
#endif
		}
...
#ifndef WE_ARE_LITTLE_ENDIAN
				DO_SWAP_LONG(faces[i][0]);
				DO_SWAP_LONG(faces[i][1]);
				DO_SWAP_LONG(faces[i][2]);
#endif
			}
		}
	}
